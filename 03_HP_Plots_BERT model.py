...
# einlesen
for i in range(1, 8):
    file_path = f'03_HP{i}_BERT_GoEmo.csv'
...
# pro Satz
# emotionen auswählen und plotten (mehrere linien)

# books = [HP1_df, HP2_df, HP3_df, HP4_df, HP5_df, HP6_df, HP7_df]
# titles = ['HP1', 'HP2', 'HP3', 'HP4', 'HP5', 'HP6', 'HP7']

# # hier aufpassen wegen ändereung von books

# fig, axes = plt.subplots(nrows=7, ncols=1, figsize=(14, 20), sharex=False)

# for i, (book, title) in enumerate(zip(books, titles)):
#     book['joy'] = book['emotions'].apply(lambda x: x.get('joy', 0))
#     book['fear'] = book['emotions'].apply(lambda x: x.get('fear', 0))
#     book['sadness'] = book['emotions'].apply(lambda x: x.get('sadness', 0))
#     book['anger'] = book['emotions'].apply(lambda x: x.get('anger', 0))

#     axes[i].plot(book.index, book['joy'], label='Joy', color='green', alpha=0.6)
#     axes[i].plot(book.index, book['fear'], label='Fear', color='red', alpha=0.6)
#     axes[i].plot(book.index, book['sadness'], label='Sadness', color='blue', alpha=0.6)
#     axes[i].plot(book.index, book['anger'], label='Anger', color='yellow', alpha=0.6)
#     axes[i].set_title(f'Sentiment {title}')
#     axes[i].set_ylabel('Emotion value/prob')
#     axes[i].legend()
#     axes[i].grid(True)
...
EMOTION_LABELS = ['joy', 'fear', 'sadness', 'anger', 'disgust', 'surprise', 'neutral','approval','annoyance']
...
for i in range(7):
    ...
    for emotion, color in zip(['joy', 'approval', 'fear', 'sadness', 'anger','annoyance'], ['green', 'lightgreen', 'blue', 'grey', 'red','orange']):
...
# emotionen zusammenfassen nach hierarchischem Clustering: positiv, negativ, neutral
...
# Gemeinsame Legende oberhalb aller Plots
handles, labels = axes[0].get_legend_handles_labels()
...
for i, (df, title) in enumerate(zip(dfs, titles)):
    ...
    # Sortiere emotions anhand des Rangs
...
for i, (df, title, chapter_list) in enumerate(zip(dfs, titles, chapters_all)):
    ...
    # Mittelwerte berechnen pro Kapitel für jede Emotion
    chapter_means = df_full.groupby('chapter', sort=False)[EMOTION_LABELS_WO_NEUTRAL].mean()

    # Jetzt pro Kategorie summieren
